...
import { uploadDirConfigs, profileDirConfigs } from '../database/config.js';
...
describe('User update', () => {
  ...
  it('returns 200 OK when update request is sent from authorized user', async () => {
    const savedUser = await addUser();
    const validUpdate = { username: 'user1-updated' };
    const response = await putUser(savedUser.id, validUpdate, {
      auth: { email: savedUser, password: 'P4ssword' },
    });
  ...
  `(
    'returns bad request with $message when username is updated with $value when language is set $language',
  ...
  it('returns 400 when image size exceeds 2mb', async () => {
    ...
    const savedUser = await addUser();
    const invalidUpdate = { username: 'update-user', image: base64 };
    const response = await putUser(savedUser.id, invalidUpdate, {
      auth: { email: savedUser.email, password: 'P4ssword' },
    });
    expect(response.status).toBe(400);
