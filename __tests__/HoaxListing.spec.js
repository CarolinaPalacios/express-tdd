...
describe('Listing All Hoaxes', () => {
  ...
  it('returns only id, content, timestamps and user object having id, username, email and image in content array for each hoax', async () => {
    await addHoaxes(11);
    const response = await getHoaxes();
    const hoax = response.body.content[0];
    const hoaxKeys = Object.keys(hoax);
    const userKeys = Object.keys(hoax.user);
    expect(hoaxKeys).toEqual(['id', 'content', 'timestamps', 'user']);
  ...
  it('returns hoaxes to be ordered from new to old', async () => {
    ...
    const lastHoax = response.body.content[response.body.content.length - 1];
    expect(firstHoax.timestamps).toBeGreaterThan(lastHoax.timestamps);
  });
});
...
describe('Listing Hoaxes of a User', () => {
  ...
  const getHoaxes = (userId) => {
    return request(app).get(`/api/1.0/users/${userId}/hoaxes`);
  };
  ...
  it('returns 10 hoaxes in page content where there are 11 hoaxes in database', async () => {
    const user = await addUser();
  ...
  it('returns 5 belongs to user in page content when there are total 11 hoaxes for two users', async () => {
    const user = await addUser();
  ...
  it('returns fileAttachment having filename and fileType if hoax has any', async () => {
    const user = await addUser();
  ...
  it('returns first page when page is set as 0 in request parameter', async () => {
    const user = await addUser();
    await addHoaxes(11, user.id);
    const response = await getHoaxes(user.id).query({ page: 0 });
    expect(response.body.page).toBe(0);
  });
